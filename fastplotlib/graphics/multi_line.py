...
class MultiLineGraphic(Graphic):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        data: array-like
            ...
            Note: 2D arrays are interpreted as y-only multi-line data. Explicit
            x/y[/z] input must use the 3D form ``(n_lines, n_points, 2|3)``;
            wrap a single explicit line as ``(1, n_points, 2|3)``.
        ...
        **kwargs
        ...
        self._data.add_event_handler(self._handle_data_change)
    ...
    def _handle_data_change(self, event) -> None:
        key = event.info["key"]
        if self._is_y_only_key(key):
            return

        self._backend.sync_x_state()

    @staticmethod
    def _is_y_only_key(key) -> bool:
        ...
        if isinstance(dims, slice):
            start, stop, step = dims.indices(3)
            return tuple(range(start, stop, step)) == (1,)
