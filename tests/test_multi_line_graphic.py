...
from fastplotlib.graphics._multi_line_scroll import (
    MultiLineScrollMaterial,
    MultiLineScrollShader,
)
...
def test_multi_line_scroll_material_and_head():
    ...
    fig = fpl.Figure()
    graphic = fig[0, 0].add_multi_line(
        data, scroll=True, scroll_head=3, scroll_n_valid=5
    )
    line = graphic.world_object.children[0]
...
def test_multi_line_scroll_shader_uses_valid_count_in_remap():
    ...
    helpers = MultiLineScrollShader(line).get_line_access_helpers()
    assert "(col + u_material.scroll_head + n_valid) % n_points" in helpers
    assert "return gfx_map_node_index(i);" in helpers
...
def test_multi_line_append_y_startup_respects_nonzero_head():
    ...
    fig = fpl.Figure()
    graphic = fig[0, 0].add_multi_line(
        data, scroll=True, scroll_head=5, scroll_n_valid=0
    )
    ...
    npt.assert_allclose(
        graphic.data.value[:, 5, 1], np.array([1.0, 10.0], dtype=np.float32)
    )
    npt.assert_allclose(
        graphic.data.value[:, 0, 1], np.array([2.0, 20.0], dtype=np.float32)
    )
...
def test_multi_line_partial_x_update_refreshes_shared_x_buffer():
    data = make_data(n_lines=2, n_points=5)
    fig = fpl.Figure()
    graphic = fig[0, 0].add_multi_line(data, scroll=False)
    line = graphic.world_object.children[0]

    new_x = np.linspace(-2, 2, graphic.n_points, dtype=np.float32)
    graphic.data[:, :, 0] = new_x

    npt.assert_allclose(graphic.data.x_values_shared, new_x)
    npt.assert_allclose(line.material.scroll_x_buffer.data, new_x)
    assert line.material.scroll_use_shared_x is True


def test_multi_line_partial_x_update_to_non_shared_disables_shared_x_buffering():
    data = make_data(n_lines=2, n_points=5)
    fig = fpl.Figure()
    graphic = fig[0, 0].add_multi_line(data, scroll=False)
    line = graphic.world_object.children[0]

    graphic.data[1, :, 0] = np.array([0.0, 1.5, 2.5, 4.0, 6.0], dtype=np.float32)

    assert graphic.data.x_values_shared is None
    assert line.material.scroll_use_shared_x is False
    npt.assert_allclose(
        line.material.scroll_x_buffer.data,
        np.arange(graphic.n_points, dtype=np.float32),
    )
